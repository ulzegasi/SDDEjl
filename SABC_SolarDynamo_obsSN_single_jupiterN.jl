...
##### Distance #####
function f_dist(θ)
	data = sn(θ; Twarmup = 200, Tobs = Tobs_without_warmup, mod = :N)
	ss = sum_stats(data)
	# Distance
...
if from_previous == 0			
	# --- Output file names ---
	fname = "obsSN_single_JN_1"
	# --- Run ---
	out = sabc(f_dist, prior; n_particles = 1000, n_simulation = 50_000_000, checkpoint_display = 500, v = 1.0, type = "single")
	display(out); flush(stdout)
elseif from_previous == 1
	# --- Output file name ---
	fname = "obsSN_single_JN_2"
	# --- Continue from... ---
	fname_previous = "obsSN_single_JN_1"
	# --- Run ---
	fasa = deserialize(outdir * "SABCresult_" * fname_previous)
	out = update_population!(fasa, f_dist, prior; n_simulation = 50_000_000, checkpoint_display = 500, v = 1.0, type = "single")
	display(out); flush(stdout)
end
