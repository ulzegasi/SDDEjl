...
if from_previous == 0
	# --- Output file names ---
	fname = "obsSN_multi_JT_1"
	# --- Run for single distance ---
	out = sabc(f_dist, prior; n_particles = 1000, n_simulation = 100_000_000, checkpoint_display = 500, v = 10.0, type = "multi")
	display(out); flush(stdout)
elseif from_previous == 1
	# --- Output file name ---
	fname = "obsSN_multi_JT_2"
	# --- Continue from... ---
	fname_previous = "obsSN_multi_JT_1"
	# --- Run ---
	fasa = deserialize(outdir * "SABCresult_" * fname_previous)
	out = update_population!(fasa, f_dist, prior; n_simulation = 100_000_000, checkpoint_display = 500, v = 10.0, type = "multi")
	display(out); flush(stdout)
end	
