...
include("./SDDESolarDynamo_withPlanets_SN.jl")
using .SDDESolarDynamo_withPlanets_SN
...
SNdata_temp = Matrix(CSV.read(datadir * "silso_SN_y_202510.csv", DataFrame, header=false))[:,2]
SNyrs_temp = Matrix(CSV.read(datadir * "silso_SN_y_202510.csv", DataFrame, header=false))[:,1]
# ------------------------------------------------------
# IMPORTANT 1: 
# We make the observations compatible with the time window covered by the torque data 
# Torque data is avaiable only until year 1948
# We can cover the time span (1700-1948) 

SNyrs = SNyrs_temp[1:249] # 1700-1948
SNdata = SNdata_temp[1:249]
...
Nd_min = 1.0;   Nd_max = 24.0
σ_min  = 0.01;  σ_max  = 0.3
Bmax_min = 1.0; Bmax_max = 15.0
ϵ_min = 0.01; ϵ_max = 0.6
ϕ_min = -10.0; ϕ_max = 0.0
...
if from_previous == 0			
	...
	# out = sabc(f_dist, prior; n_particles = 1000, n_simulation = 400_000_000, 
	# 			show_checkpoint = 500, v = 1.0, 
	# 			algorithm = :single_eps)
	display(out); flush(stdout)
elseif from_previous == 1
	# --- Output file name ---
	fname = "obsSN_single_71b"
	# --- Continue from... ---
	fname_previous = "obsSN_single_71"
	# --- Run ---
