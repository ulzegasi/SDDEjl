...
out_singeps = sabc(f_dist, prior; n_particles = 10000, v = 1.0, n_simulation = nsim, algorithm = :single_eps)
display(out_singeps)
# --- Run for multiple distances ---
out_multeps = sabc(f_dist, prior; n_particles = 10000, v = vmulti, n_simulation = nsim, algorithm = :multi_eps)
display(out_multeps)
# --- Run old ---
out_old = sabc(f_dist_old, prior; n_particles = 10000, v = 1.0, n_simulation = nsim, algorithm = :single_eps)
display(out_old)
...
histogram!(P_post_3, pop_old[1,:], fillcolor = :skyblue1, fillalpha=0.5, 
			bins=range(μ_min/bins_range_factor, μ_max/bins_range_factor, length=bins_range_length), label="hybrid")
...
x_range = LinRange(x_min-0.1, x_max+0.1, 1000)
