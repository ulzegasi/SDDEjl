...
p1 = histogram(
    ...
    xlims = mu_lims,
    ylims = (0, 1.5),
    legend = false,              # <-- (optional) disable legend for this subplot
)
...
#= @btime sabc(
    ...
    algorithm = :single_eps,
    proposal = DifferentialEvolution(n_para = length($prior))
); =#  # <-- benchmarked run: 1.3 sec (4 therads), 3.4 sec (1 thread)
...
title!(p_sabc_singeps, "SABC, single_eps, 2 stats")
...
title!(p_sabc_multeps, "SABC, multi_eps, 2 stats")
plot!(p; colorbar = false)  # ensure equal panel widths
...
display(plot_sabc_trajectories(
    ...
    algorithm = :single_eps,
    stat_labels = ["μ", "σ"],
    title_str = "Algorithm: single_eps | Summary stats: μ, σ",
	size_px = (700, 900)
))
...
display(plot_sabc_trajectories(
    ...
    algorithm = :multi_eps,
    stat_labels = ["μ", "σ"],
    title_str = "Algorithm: multi_eps | Summary stats: μ, σ",
	size_px = (700, 900)
))
