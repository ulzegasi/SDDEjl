...
# --------------------------
# fourier_range = [65, 1, 165, 8, 47, 120, 155, 156] .+ 1  # without Jupiter
# I TAKE OUT 120 FROM ABOVE BECAUSE IT IS ALWAYS EXACTLY = 0 AND GIVES NaN IN THE CDF TO CALCULATE THE u's 
# fourier_range = [65, 1, 165, 8, 47, 155, 156] .+ 1  # without Jupiter
# --------------------------
# we can try with a different set of summary stats
# that does not cotain 120 and is already at the minimum of the ii curve
# fourier_range = [1, 168, 169, 47, 20, 158] .+ 1  # without Jupiter
# --------------------------
# After replacing OU noise with white noise, we get a different set of summary stats:
fourier_range = [97, 2, 234, 235, 236, 206, 144, 114, 152, 28, 216] .+ 1  # without Jupiter
# --------------------------
# fourier_range = [1, 233, 235, 109, 78, 111, 239, 238, 113] .+ 1  # with Jupiter
...
function f_dist_singeps(θ)
        ...
        rho = [euclidean(ss, ss_obs), [euclidean(ss[ix], ss_obs[ix]) for ix in 1:num_sumstats]]  
        return rho
end
...
# --- Output file names ---
fname_singeps = "recSN_singeps_ii_noJ_10"
fname_multeps = "recSN_multeps_ii_noJ_10"
...
fname_singeps = "recSN_singeps_ii_noJ_11"
...
# out_multeps_1 = sabc(f_dist_multeps, prior; n_particles = 1000, n_simulation = 6_000_000, checkpoint_display = 100)
